...
interface IERC165 => transferが生えているか聞けるメソッド
interface IERC721 is IERC165 => balanceOf ownerOf approve
interface IERC721Metadata is IERC721 => name,symbol,tokenURI
interface IERC721Enumerable is IERC721 => totalSupply tokenOfOwnerByIndex,tokenByIndex
interface IERC721Receiver => safeTransferFromを介してトークンを受け入れる場合、コントラクトによって実装される必要があるインターフェース


# https://docs.openzeppelin.com/contracts/3.x/api/utils
# ライブラリは指定したデータ型に対して、ライブラリで定義したメソッドを追加することができる
# https://qiita.com/ryu-yama/items/0d248605c7447638df64

library SafeMath => 四則演算の関数とかが入ってるライブラリ
library Address => アドレス型にメソッドを生やすライブラリ
library EnumerableSet => ?
library EnumerableMap => ハッシュ型にメソッドを生やすライブラリ?
library Strings => toString
...
Solidity の算術演算のラッパーで、オーバーフローチェックが追加されています。
Solidity の算術演算はオーバーフローでラップされます。なぜなら、プログラマは通常、オーバーフローはエラーを発生させると考えるからです。これは、高レベルのプログラミング言語では標準的な動作です。
SafeMath は、演算がオーバーフローしたときにトランザクションを戻すことで、この直観を復元します。演算がオーバーフローした場合、トランザクションを元に戻すことによって、この直感を復元する。
チェックされていない演算の代わりにこのライブラリを使用することで、ある種のバグを排除することができます。そのため、常にこのライブラリを使用することをお勧めします。
